...
def main() -> int:
    ...
    os.environ.update({k: env[k] for k in ("AGENT_DB", "PLACEMENT_DB", "PYTHONUTF8", "PYTHONIOENCODING")})
    ...
    print(
        f"3. killed worker-A after it sent the notification but before it recorded doing so: run is '{run['status']}',"
