...
class RunStore:
    ...
    def enqueue(self, thread_id: str, text: str, model: str, max_attempts: int = 3) -> str:
        ...
        with self.transaction() as c:
            ...
            c.execute(
                "INSERT INTO run (id, thread_id, status, model, max_attempts, available_at, attempts) "
                "VALUES (?, ?, 'queued', ?, ?, ?, 0)",
    ...
    def claim_next(self, worker_id: str, lease_seconds: float) -> Claimed | None:
        ...
        with self.transaction() as c:
            ...
            started_clause = ""
            params = [worker_id, lease_until, new_attempts]
            if row["started_at"] is None:
                started_clause = f", started_at = {NOW_SQL}"

            c.execute(
                f"UPDATE run SET status = 'running', lease_owner = ?, lease_until = ?, "
                f"attempts = ?{started_clause} WHERE id = ?",
                (*params, run_id),
            )
        ...
        with self.transaction() as c:
            ...
            for r in rows:
                ...
                if r["attempts"] < r["max_attempts"]:
                    c.execute(
                        "UPDATE run SET status = 'queued', available_at = ?, "
                        "lease_owner = NULL, lease_until = NULL, error_code = 'lease_expired' "
                        "WHERE id = ?",
                        (now, run_id),
                    )
                else:
                    c.execute(
                        f"UPDATE run SET status = 'dead', lease_owner = NULL, lease_until = NULL, "
                        f"error_code = 'lease_expired', finished_at = {NOW_SQL} WHERE id = ?",
                        (run_id,),
                    )
            return touched
    ...
    def fail_attempt(self, run_id: str, worker_id: str, error_code: str, retryable: bool,
        ...
        with self.transaction() as c:
            row = c.execute(
                "SELECT attempts, max_attempts FROM run "
                "WHERE id = ? AND status = 'running' AND lease_owner = ?",
                (run_id, worker_id),
            ).fetchone()
            if row is None:
            ...
            if not retryable:
                c.execute(
                    f"UPDATE run SET status = 'failed', error_code = ?, lease_owner = NULL, "
                    f"lease_until = NULL, finished_at = {NOW_SQL} WHERE id = ?",
                    (error_code, run_id),
                )
                return "failed"
            ...
            if attempts < max_attempts:
                ...
                c.execute(
                    ...
                    (avail_at, error_code, run_id),
                )
                return "queued"

            c.execute(
                f"UPDATE run SET status = 'dead', error_code = ?, lease_owner = NULL, "
                f"lease_until = NULL, finished_at = {NOW_SQL} WHERE id = ?",
                (error_code, run_id),
            )
            return "dead"