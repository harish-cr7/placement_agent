...
class PlacementDb:
    ...
    def create_application(self, student_id: int, drive_id: int) -> tuple[int, bool]:
        ...
        Uses ON CONFLICT ... DO NOTHING, returning the existing row's id with created = False.
        """
        with self.transaction() as c:
            cur = c.execute(
    ...
    def claim_slot(self, slot_id: int, student_id: int, expected_version: int) -> bool:
        ...
        Optimistic locking: checks free status and version match, then bumps version by 1.
        """
        with self.transaction() as c:
            cur = c.execute(
    ...
    def record_notification(self, roll_no: str, message: str, dedupe_key: str) -> tuple[int, bool]:
        ...
        Stores only if no notification has this dedupe_key; otherwise returns existing id.
        """
        with self.transaction() as c:
            cur = c.execute(
    ...
    def once(self, key: str, tool_name: str, effect: Callable[[], dict]) -> tuple[dict, bool]:
        """Runs effect at most once per key.
        If key already in idempotency -> return (stored result, False).
